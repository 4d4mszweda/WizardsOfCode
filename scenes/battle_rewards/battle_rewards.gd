...
@export var run_status: RunStats
...
func _show_card_rewards() -> void:
	if not run_status or not character_stats:
		return
	...
	for i in run_status.card_rewards:
		_setup_card_chances()
...
func _setup_card_chances() -> void:
	card_reward_total_weight = run_status.common_weight + run_status.uncommon_weight + run_status.rare_weight
	card_rarity_weights[Card.Rarity.COMMON] = run_status.common_weight
	card_rarity_weights[Card.Rarity.UNCOMMON] = run_status.common_weight + run_status.uncommon_weight
...
func _modify_weights(rarity_rolled: Card.Rarity) -> void:
	if rarity_rolled == Card.Rarity.RARE:
		run_status.rare_weight = RunStats.BASE_RARE_WEIGHT
	else:
		run_status.rare_weight = clampf(run_status.rare_weight + 0.3, run_status.BASE_RARE_WEIGHT, 5.0)
