...
const CARD_MENU_UI_SCENE := preload("res://scenes/ui/card_menu.tscn")

@onready var tooltip_card: CenterContainer = %TooltipCard
@onready var card_desription: RichTextLabel = %CardDescription
...
func show_tooltip(card: Card) -> void:
	...
	card_desription.text = card.tooltip_text
	show()
